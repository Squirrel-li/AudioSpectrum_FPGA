...
\subsection{FLASH Slot 與播放流程}

本系統將 FLASH 分成四個 slot，由 SW[11:10] 選擇目標 slot。儲存時，系統先確認 SW3 已解鎖，再將 SRAM 中的錄音資料與 header 寫入指定 slot。播放時，系統一律先讀取 FLASH header，確認有效後將資料載入 SRAM，再由 SRAM 連續輸出音訊，避免直接從 FLASH 串流造成播放時序不穩定。

\subsection{LEDR 音量顯示}

完整 FFT 運算與外接 LED 矩陣不列入本版本功能。本系統改以音訊 sample 振幅控制 LEDR[17:0]，聲音越大亮起的 LED 數越多。此方式硬體成本低、延遲小，且足以呈現錄音或播放時的即時音量變化。
...
\begin{figure}[H]
\centering
\fbox{\parbox[c][5cm][c]{10cm}{\centering 請放入 LEDR 音量條畫面}}
\caption{LEDR 音量條顯示}
\end{figure}
